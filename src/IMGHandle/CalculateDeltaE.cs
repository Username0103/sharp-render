// stolen from https://github.com/muak/ColorMinePortable/tree/master
...
namespace sharp_render.src.IMGHandle
{
    ...
    {
        public double CalculateDeltaE(Color c1, Color c2)
        {
            var c1RGB = new Rgb { R = c1.R, B = c1.B, G = c1.G };
            var c2RGB = new Rgb { R = c2.R, B = c2.B, G = c2.G };
            return new CIEDE2000Calculator().Compare(c1RGB, c2RGB);
        }
        public class CIEDE2000Calculator : IColorSpaceComparison
        {
            public double Compare(IColorSpace c1, IColorSpace c2)
            {
                ...
                double G = 0.5d * (1 - Math.Sqrt(c_star_average_ab_pot7 / (c_star_average_ab_pot7 + 6103515625)));
